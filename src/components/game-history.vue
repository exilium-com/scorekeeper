...
let indexedGameHistory = computed(() => {
...
<template>
    <v-list-item v-for="game in indexedGameHistory.slice(props.start, props.end).reverse()"
        @click="scoreStore.changeGame(game.index)" :active="game.index === scoreStore.curGame" :key="game.index">
