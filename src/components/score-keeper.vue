...
watch(
    () => [scoreStore.rounds, scoreStore.players],
    () => {
...
function isRoundWinner(scoreIndex: number, roundIndex: number) {
    ...
    const maxScore = Math.max(
        ...scoreStore.rounds[roundIndex].scores.map((score) => (typeof score.score !== 'number' ? Number.MIN_SAFE_INTEGER : score.score))
    )
...
function isWinner(playerIndex: number) {
    ...
    const maxScore = Math.max(...scoreStore.playersTotal.map((total) => (typeof total !== 'number' ? Number.MIN_SAFE_INTEGER : total)))
    ...
    <v-table fixed-header height="80vh" density="comfortable">
        ...
        <tbody>
            <tr v-for="(round, roundIndex) in scoreStore.rounds" :key="roundIndex">
                <td class="round-color d-flex align-center justify-center">
                    {{ scoreStore.rounds.length > 1 ? roundIndex + 1 : '' }}
                    <v-icon
                        v-if="isEditing"
                ...
                <td v-for="(score, scoreIndex) in round.scores" :key="scoreIndex">
                    <v-text-field
                        ...
                        :class="{
                            'winner-color-text-field': isRoundWinner(scoreIndex, roundIndex),
